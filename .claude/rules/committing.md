...
- **One command per Bash call** — never chain git commands with `&&`,
  `;`, heredocs, or subshells. This rule comes from `shell.md` and
  applies to ALL Bash calls, including git operations during the
  commit workflow. Each `git add`, `git commit`, `git push`, etc.
  must be its own separate Bash call.
