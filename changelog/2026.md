...
- Refactored `evaluate_answer()` in `exercises.py`: merged duplicated
  ...
  first.
- `db.py`:
  - Refactored `authenticate_user()`: delegates to `get_user()` instead
    of duplicating the username lookup query.
  - Extracted `_in_clause()` helper: replaces repeated
    `",".join("?" for _ in ids)` pattern in `get_all_progress()` and
    `get_lessons()`.
  - Standardised datetime parsing: replaced all `strptime()` calls with
    `fromisoformat()` for reading timestamps.
...
- `scripts/`:
  - Added `topic_classifier.py`: keyword-based topic classifier
    with 12 topic categories (food, travel, body, emotions, family,
    home, nature, work, time, clothing, health, education).
  - Updated `build_spanish_vocab.py`: classifies each gloss and
    includes `tags` in the output JSON.
  - Updated `build_spanish_definitions.py`: carries `tags` from the
    bilingual file into monolingual output.
