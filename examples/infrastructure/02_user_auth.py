...
async def main() -> None:
    ...
    print(
        f"Registered: {alice.username} "
        f"(id={alice.id})"
    )
    print(
        f"Registered: {bob.username} "
        f"(id={bob.id})"
    )
    ...
    print(
        f"Bad password: "
        f"{'rejected' if bad is None else 'BUG'}"
    )

    # --- Session tokens ---
    session = await db.create_session(
        alice.id, ttl_hours=1,
    )
    print(
        f"\nSession token: "
        f"{session.token[:16]}..."
    ...
    print(
        f"Expires at: "
        f"{session.expires_at:%Y-%m-%d %H:%M}"
    )
