...
async def test_authenticate_user_wrong_password(db):
    await db.register_user("alice", "s3cret")
    assert (await db.authenticate_user("alice", "wrong")) is None


async def test_authenticate_user_unknown_username(db):
    assert (await db.authenticate_user("ghost", "pass")) is None
...
async def test_add_concept_with_context(db):
    ...
    assert loaded[0].context == "A common household pet"
...
async def test_get_answer_history_since(db):
    ...
    await db._conn.execute(
        ...
        "VALUES (?, ?, ?, ?, ?, ?)",
        (u.id, 1, "flashcard", 1, 5, "2026-01-01T00:00:00"),
    )
    await db._conn.execute(
        "INSERT INTO answer_history "
        "(user_id, concept_id, exercise_type, "
        " correct, quality, answered_at) "
        "VALUES (?, ?, ?, ?, ?, ?)",
        (u.id, 2, "flashcard", 1, 5, "2026-06-01T00:00:00"),
    )
...
async def test_daily_stats_aggregation(db):
    ...
    await db._conn.execute(
        ...
        " correct, quality, answered_at) "
        "VALUES (?, ?, ?, ?, ?, ?)",
        (u.id, 1, "flashcard", 1, 5, "2026-02-27T10:00:00"),
    )
    await db._conn.execute(
        "INSERT INTO answer_history "
        "(user_id, concept_id, exercise_type, "
        " correct, quality, answered_at) "
        "VALUES (?, ?, ?, ?, ?, ?)",
        (u.id, 2, "flashcard", 0, 1, "2026-02-27T11:00:00"),
    )
    await db._conn.commit()
...
async def test_daily_stats_multiple_days(db):
    ...
    for day, cid in [
        ...
        await db._conn.execute(
            ...
            " correct, quality, answered_at) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (u.id, cid, "flashcard", 1, 5, f"{day}T10:00:00"),
        )
    await db._conn.commit()
    stats = await db.daily_stats(u.id, days=365)
    assert len(stats) == 2
